...
public:
    ...
    virtual stream_pointer_t getInputStream(
        std::string const& entry_name, MatchPath matchpath = MatchPath::MATCH) override;
    static void saveCollectionToArchive(
        std::ostream& os, FileCollection& collection, std::string const& zip_comment = std::string());
